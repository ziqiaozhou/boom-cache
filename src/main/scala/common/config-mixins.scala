...
import freechips.rocketchip.subsystem.{SystemBusKey, RocketTilesKey, RocketCrossingParams,WithNoSlavePort,WithNoMMIOPort,WithoutTLMonitors}
 ...
 * 1-wide BOOM.
 */
class WithMyTinyBooms(nsets: Int, nways: Int) extends Config((site, here, up) => {
  new WithNoMMIOPort ++
  new WithNoSlavePort ++
  new WithoutTLMonitors ++
  WithMyTinyBoomsInternal(nsets,nways) ++
}
class WithMyTinyBoomsInternal(nsets: Int, nways: Int) extends Config((site, here, up) => {
  case BoomTilesKey => up(BoomTilesKey, site) map { b => b.copy(
    ...
    dcache = Some(DCacheParams(rowBits = site(SystemBusKey).beatBits,
                               nSets=nsets, nWays=nways, nMSHRs=2, nTLBEntries=4)),
